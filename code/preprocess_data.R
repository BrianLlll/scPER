...
sc_df<-read.csv(args[1],row.names = 1,sep='\t')

bulk<-read.csv(args[2],row.names = 1,sep='\t')
...
write.csv(mad5000,'reference_top5k_imputation.csv',quote=FALSE)


bulk_overlap<-bulk[mad.genes,]

write.csv(bulk_overlap,'bulk_5k_genes_matched.csv',quote=FALSE)
